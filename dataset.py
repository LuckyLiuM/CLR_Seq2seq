...
class CSL_Continuous(Dataset):
    def __init__(self, data_path, dict_path, corpus_path, frames=20, train=True, transform=None):
        ...
        self.signers = 50 ##一个文件夹有五个手语者
        self.repetition = 5 ##重复五遍
        if self.train:  ##划分数据集
            self.videos_per_folder = int(0.8 * self.signers * self.repetition)
        else:
            self.videos_per_folder = int(0.2 * self.signers * self.repetition)
        # dictionary
        self.dict = {'<pad>': 0, '<sos>': 1, '<eos>': 2} ##pad补全字符 sos起始字符 eos结束字符
        self.output_dim = 3 ##输出向量的维度
        try:
            ...
            raise
        # img data
        self.data_folder = [] ##？？ 000
        try:
            obs_path = [os.path.join(self.data_path, item) for item in os.listdir(self.data_path)] ##拼接图片的地址
            self.data_folder = sorted([item for item in obs_path if os.path.isdir(item)])   ##判断是否是目录
        except Exception as e:
            raise
        # corpus
        self.corpus = {}
    ...
    def read_images(self, folder_path): #000
        assert len(os.listdir(folder_path)) >= self.frames, "Too few images in your data folder: " + str(folder_path)+"文件长度"+str(len(os.listdir(folder_path)))
        ##如果folder path 下照片过少，进行提示
        images = []
        start = 1
        step = int(len(os.listdir(folder_path))/self.frames) ##间距 如果是2的话就隔一个取样
        for i in range(self.frames):  ##这里可不可以取frame和照片的最小值？
            image = Image.open(os.path.join(folder_path, '{:06d}.jpg'.format(start+i*step)))  #.convert('L')
            # image.show()
            if self.transform is not None:
                image = self.transform(image)
...
class CSL_Continuous_Char(Dataset):
    def __init__(self, data_path, corpus_path, frames=128, train=True, transform=None):
        ...
        self.transform = transform
        self.num_sentences = 100
        self.signers = 50 ##一个文件夹里的文件？
        self.repetition = 5 ##有五个文件夹
        if self.train:      ##划分数据集
            self.videos_per_folder = int(0.8 * self.signers * self.repetition)
        else:
            self.videos_per_folder = int(0.2 * self.signers * self.repetition)
...
if __name__ == '__main__':
    ...
    dataset = CSL_Continuous(
        data_path="E:/pythonwork/cv/SLR_videos_frm_cut1",
        dict_path="E:/pythonwork/cv/dictionary.txt",
        corpus_path="E:/pythonwork/cv/corpus.txt",
